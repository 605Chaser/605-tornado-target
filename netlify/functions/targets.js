...
async function fetchNAM(lat, lon, day) {
  ...
  const url = `https://api.open-meteo.com/v1/gfs?latitude=${lat}&longitude=${lon}` +
    ...
    `wind_direction_10m,wind_direction_80m,temperature_2m,dew_point_2m` +
...
function scoreBestWindow(hourly, day, targetTz) {
  ...
  // For day 1 use today's date, day 2 use tomorrow
  const targetDate = new Date(now);
  if (day === 2) targetDate.setUTCDate(targetDate.getUTCDate() + 1);
  const targetDateStr = targetDate.toISOString().slice(0, 10);

  // For day 1, only score hours from now forward
  // For day 2, score all hours on that date
  ...
  times.forEach((t, i) => {
    ...
    // Day 1: skip hours already passed, and skip overnight (midnight-noon)
    if (day === 1) {
      if (hour < currentUTCHour) return;
      if (hour < 12 && hour >= 0) return;
    }
    // Day 2: only score convective window 12Z-06Z next day
    if (day === 2 && hour < 12 && hour > 6) return;
  ...
  const startUTC   = Math.max(bestUTCHour - 2, 0);
  const endUTC     = (bestUTCHour + 2) % 24;
  ...
  // Determine which model run this data is from
  const runHour = now.getUTCHours();
  const modelRun = runHour >= 18 ? '18Z' : runHour >= 12 ? '12Z' : runHour >= 6 ? '06Z' : '00Z';
